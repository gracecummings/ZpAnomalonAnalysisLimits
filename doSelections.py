...
if __name__=='__main__':
    ...
    for fjec in inputfiles:
        ...
        rootfilename  = go.makeOutFile(samp,'upout_'+region+'_'+jectype+'_'+btaggr,'.root',str(zptcut),str(hptcut),str(metcut),str(btagwp))#need to update for btagger
        npfilename    = go.makeOutFile(samp,'totalevents_'+region+'_'+jectype+'_'+btaggr,'.npy',str(zptcut),str(hptcut),str(metcut),str(btagwp))
        pklfilename   = go.makeOutFile(samp,'selected_errors_'+region+'_'+jectype+'_'+btaggr,'.pkl',str(zptcut),str(hptcut),str(metcut),str(btagwp))
        ...
        rootOutFile["h_zp_jigm"]    = np.histogram(fdf['ZPrime_mass_est'],bins=46,range=(500,5100),weights=fdf['event_weight'])
