    ...
    if len(fsjson.keys()) > 0:
        for key in fsjson.keys():
            for samp in fsjson[key]:
                ...
                #samplistasstr = str(fullpaths[588:586]).replace(' ','')
                #print("HAMPERED THE FILELIST FOR DEBUG -- REMOVE TRUNCATION")
                #print(samplistasstr)
                #sampleName = sampleName+"_2v2g1e"
                ...
                if not args.killsubmission:
                    ...
                    jdlName = "topiary_"+sampleName+"_"+syststr+"_"+str(date.today())+".jdl"
                    jdl = open(jdlName,"w")
