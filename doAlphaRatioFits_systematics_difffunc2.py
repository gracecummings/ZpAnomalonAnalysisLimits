...
def getTheShiftedExtrapolationsExpN(noma,nomsub,shifta,shiftsub,direc):
    shiftedExtraps = {}
    for key in shiftsub.keys():
        name = "extrap_"+key
        fit = shiftsub[key]
        shiftedExtraps[name] = ROOT.alphaExtrapolationFromFitsExpN(fit,noma,name)
    for key in shifta.keys():
        name = "extrap_"+key
        fit = shifta[key]
        shiftedExtraps[name] = ROOT.alphaExtrapolationFromFitsExpN(nomsub,fit,name)
    return shiftedExtraps

def getTheShiftedExtrapolationsExpSq(noma,nomsub,shifta,shiftsub,direc):
    shiftedExtraps = {}
    for key in shiftsub.keys():
        name = "extrap_"+key
        fit = shiftsub[key]
        shiftedExtraps[name] = ROOT.alphaExtrapolationFromFitsExpSq(fit,noma,name)
    for key in shifta.keys():
        name = "extrap_"+key
        fit = shifta[key]
        shiftedExtraps[name] = ROOT.alphaExtrapolationFromFitsExpSq(nomsub,fit,name)
    return shiftedExtraps
...
def getShiftedSubtractionsExpN(sbdatfitfit,nomdat,nomtt,nomvv,shiftdata,shifttt,shiftvv,direc):
    newsubdatasbs = {}
    listoffits = []
    for i,fit in enumerate(shiftdata):
        name = "subdatasb_datafit_par"+str(i)+direc
        ssub = ROOT.subtractionFromFitsExpN(fit,nomtt,nomvv,name)
        newsubdatasbs[name] = ssub
    for i,fit in enumerate(shifttt):
        name = "subdatasb_ttfit_par"+str(i)+direc
        newsubdatasbs[name] = ROOT.subtractionFromFitsExpN(nomdat,fit,nomvv,name)
    for i,fit in enumerate(shiftvv):
        name = "subdatasb_vvfit_par"+str(i)+direc
        ssubvv = ROOT.subtractionFromFitsExpN(nomdat,nomtt,fit,name)
        newsubdatasbs[name] = ssubvv
    return newsubdatasbs

def getShiftedSubtractionsExpSq(sbdatfitfit,nomdat,nomtt,nomvv,shiftdata,shifttt,shiftvv,direc):
    newsubdatasbs = {}
    listoffits = []
    for i,fit in enumerate(shiftdata):
        name = "subdatasb_datafit_par"+str(i)+direc
        ssub = ROOT.subtractionFromFitsExpSq(fit,nomtt,nomvv,name)
        newsubdatasbs[name] = ssub
    for i,fit in enumerate(shifttt):
        name = "subdatasb_ttfit_par"+str(i)+direc
        newsubdatasbs[name] = ROOT.subtractionFromFitsExpSq(nomdat,fit,nomvv,name)
    for i,fit in enumerate(shiftvv):
        name = "subdatasb_vvfit_par"+str(i)+direc
        ssubvv = ROOT.subtractionFromFitsExpSq(nomdat,nomtt,fit,name)
        newsubdatasbs[name] = ssubvv
    return newsubdatasbs
...
def getShiftedAlphaExpNRatios(sbnom,srnom,sbshifts,srshifts,direc):
    newalphas = {}
    #do shifted sideband fits
    for i,fit in enumerate(sbshifts):
        name = "alpha_DY_sb_par"+str(i)+direc
        salpha = ROOT.alphaRatioMakerExpNExternalParams(fit,srnom,name)
        newalphas[name] = salpha
    for i,fit in enumerate(srshifts):
        name = "alpha_DY_sr_par"+str(i)+direc
        salpha = ROOT.alphaRatioMakerExpNExternalParams(sbnom,fit,name)
        newalphas[name] = salpha
    return newalphas
...
def doExpNFitShifts(nomfit,parnum,name,lowr,highr,shiftedparamsin = ROOT.TVector()):
    print("doing the shifted fits!")
    fitpars = []
    fiterrs = []
    shiftedfits = []
    for i in range(parnum):
        ...
        for j in range(parnum):
            parvecedit[j] = shiftedparamsin[i][j]
        print(parvecedit[0],parvecedit[1],parvecedit[2])
        fit = ROOT.expNFitSetParsAndErrs(name+"par"+str(i),parvecedit,lowr,highr)
        shiftedfits.append(fit)

    return fitpars,fiterrs,shiftedfits

def doExpSqFitShifts(nomfit,parnum,name,lowr,highr,shiftedparamsin = ROOT.TVector()):
    print("doing the shifted fits!")
    fitpars = []
    fiterrs = []
    shiftedfits = []
    for i in range(parnum):
        ...
        for j in range(parnum):
            parvecedit[j] = shiftedparamsin[i][j]
        print(parvecedit[0],parvecedit[1],parvecedit[2])
        fit = ROOT.expSqFitSetParsAndErrs(name+"par"+str(i),parvecedit,lowr,highr)
        shiftedfits.append(fit)
...
if __name__=='__main__':
    ...
    if validation:
        ...
        if len(years) == 2:
            ...
            #dynorm = np.load('mumu_2022-03-31_ProperREOIDSF_AlphaMethodExtrap/Run2_161718_dynormalization_systnominal_kfnom_btagnom_muidnom_elidnom_elreconom_signalblind_Zptcut100.0_Hptcut300.0_metcut0.0_btagwp0.8.npy')[0]
            #dynorm = np.load('mumu_2022-07-18_ProperSF_EE_METXY_HEMveto_Pref/Run2_161718_dynormalization_alphat_systnominal_hem_kf_btag_muid_mutrig_eltrig_elid_elreco_signalblind_Zptcut100.0_Hptcut300.0_metcut75.0_btagwp0.8.npy')[0]
    ...
    sbttunc = ROOT.expFitErrBands(hsbtt,"sbl","QER0+",sigmabars,ttsbxmax,ttsbxmin)#3000)
    ...
    sbdatfit = ROOT.expSqFit(hdatsb,"datsbl","ER0+",datsbxmax,datsbxmin)#5000)
    sbdatunc = ROOT.expSqFitErrBands(hdatsb,"sbl","ER0+",sigmabars,datsbxmax,datsbxmin)#5000)
    datsbfitdecorrparamsup = ROOT.expSqFitDecorrParamsShiftedUp(hdatsb.Clone(),"datsbshiftfinder","ER0+",datsbxmax,datsbxmin)
    #print("The vector of shifted parameters returned to python")
    #datsbfitdecorrparamsup.Print()
    datsbparsdecoup,datsbfiterrdecoup,datsbfitsdecoup = doExpSqFitShifts(sbdatfit,3,"datsbup",datsbxmax,13000,datsbfitdecorrparamsup)
    datsbfitdecorrparamsdn = ROOT.expSqFitDecorrParamsShiftedDown(hdatsb.Clone(),"datsbshiftfinder","RE0+",datsbxmax,3100)#datsbxmin)
    datsbparsdecodn,datsbfiterrdecodn,datsbfitsdecodn = doExpSqFitShifts(sbdatfit,3,"datsbdwn",datsbxmax,13000,datsbfitdecorrparamsdn)
    ...
    subdatafit = ROOT.subtractionFromFitsExpSq(sbdatfit,sbttfit,sbvvfit,"subtracteddatasbfit")
    sbdatuncsub = castFitIntoHistogram(empty,{"subtractedSideband":subdatafit})[0]
    shiftedsubsup = getShiftedSubtractionsExpSq(subdatafit,sbdatfit,sbttfit,sbvvfit,datsbfitsdecoup,ttsbfitsdecoup,vvsbfitsdecoup,"up")
    shiftedsubsdn = getShiftedSubtractionsExpSq(subdatafit,sbdatfit,sbttfit,sbvvfit,datsbfitsdecodn,ttsbfitsdecodn,vvsbfitsdecodn,"dwn")
    print("=========doing sb extrapolation fit==================")
    #extrap  = ROOT.alphaExtrapolationExpSq(hsbdy,hsrdy,sbdatuncsub,dysbxmax,dysbxmin,dysrxmax,dysrxmin,datsbxmax,datsbxmin)
    extrap = ROOT.alphaExtrapolationFromFitsExpSq(subdatafit,alpha,"extrap")
    extrphists = castFitIntoHistogram(empty,{"extrap":extrap})
    extrphist = extrphists[0]
    #extrphist = ROOT.alphaExtrapolationHistExpSq(hsbdy,hsrdy,sbdatuncsub,1,dysbxmax,dysbxmin,dysrxmax,dysrxmin,datsbxmax,datsbxmin)
    #alpharver = ROOT.alphaExtrapolationFromFits(subdatafit,alpha,"alphaextrapnohistinput")
    shiftedextrapsup = getTheShiftedExtrapolationsExpSq(alpha,subdatafit,alphaups,shiftedsubsup,"up")
    shiftedextrapsdn = getTheShiftedExtrapolationsExpSq(alpha,subdatafit,alphadns,shiftedsubsdn,"dwn")
    ...
    plotShiftedFits(sbttfit,hsbtt,ttsbfitsdecoup,"TTSB","up",55,config.get(systname,syststr),ttsbfitgofpad,sbttunc,"decorrelatedShifts")
    plotShiftedFits(sbttfit,hsbtt,ttsbfitsdecodn,"TTSB","dwn",55,config.get(systname,syststr),ttsbfitgofpad,sbttunc,"decorrelatedShifts")
    plotShiftedFits(sbvvfit,hsbvv,vvsbfitsdecoup,"VVSB","up",5,config.get(systname,syststr),vvsbfitgofpad,sbvvunc,"decorrelatedShifts")
    plotShiftedFits(sbvvfit,hsbvv,vvsbfitsdecodn,"VVSB","dwn",5,config.get(systname,syststr),vvsbfitgofpad,sbvvunc,"decorrelatedShifts")
    plotShiftedFits(sbdatfit,hdatsb,datsbfitsdecoup,"dataSB","up",150,config.get(systname,syststr),datsbfitgofpad,sbdatunc,"decorrelatedShifts_ExpNFunction3100",isLog=True)
    plotShiftedFits(sbdatfit,hdatsb,datsbfitsdecodn,"dataSB","dwn",150,config.get(systname,syststr),datsbfitgofpad,sbdatunc,"decorrelatedShifts_ExpNFunction3100",isLog=True)
