   ...
   # inputfiles = glob.glob(topdir+'/'+samp+'*_topiary*systnominal*.root')
    ...
    for fjec in inputfiles:
        ...
        if not (valid or alphatest):
            if stype != 0:
                if sr and not alphatest:
                    fdf = srdf
                    region = "signalr"
                    print("    using signal region selections")
                elif comb and not alphatest:
                    fdf = btdf
                    region = "totalr"
                    print("    using full region selections")
                else:
        ...
        if alphatest:
            if sr:
                ...
                print("    Using a total soft drop mass region without btagging")
            else:
                fdf = sbdf
                region = "sideband_alphatest"
                print("    using a softdrop mass sideband and antibtagged")
