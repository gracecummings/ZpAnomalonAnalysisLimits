...
if __name__=='__main__':
    ...
    #bkgcounts = go.gatherBkg('pfMETNominal_CorrectCounting','totalevents_sideband_systnominal_btagnom',zptcut,hptcut,metcut,btagwp,year)
    #bkgerrfs  = go.gatherBkg('pfMETNominal_CorrectCounting','selected_errors_sideband_systnominal_btagnom',zptcut,hptcut,metcut,btagwp,year)
    ...
    daterrfs = glob.glob('analysis_output_ZpAnomalon/'+args.date+'/'+datprefix+'*selected_errors_sideband_systnominal_btagnom*_Zptcut'+str(zptcut)+'_Hptcut'+str(hptcut)+'_metcut'+str(metcut)+'_btagwp'+str(btagwp)+'.pkl')
    datadfs = []
    if len(daterrfs) > 0:
        print("Doing data uncertainties")
        for d  in daterrfs:
